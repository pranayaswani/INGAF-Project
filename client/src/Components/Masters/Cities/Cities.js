...
const Cities = () => {
    ...
    useEffect(()=>{
      axios.get(`http://localhost:5000/getById/cities/${id}`)
        .then((response) => {
    ...
    useEffect(()=>{
        ...
        if(Object.keys(formErrors).length===0 && isSubmit)
        {
            ...
            {
              axios.put(`http://localhost:5000/updateStatus/cities/${id}`,{
                status_id
                }).then((res)=>{
                  console.log(res);
                  if(res.status==200)
            ...
            {
              axios.get(`http://localhost:5000/get/city_check/${descr}`)
                        .then((res1)=>{
                        ...
                        }else
                        {
                          if(id)
                          {
                            axios.put(`http://localhost:5000/update/cities/${id}`,{
                              descr,status_id
                              }).then((res)=>{
                                if(res.status==200)
                                {
                          ...
                          {
                              axios.post("http://localhost:5000/post/cities",
                              {descr,status_id})
                              .then((res)=>
                              {
                                  if(res.status==200)
  ...
  return (
    ...
    <form className='ui form' onSubmit={handleSubmit}> 
        <div className="ui segment">
            ...
            <button className="ui button primary w-20" hidden={action? "hidden" : ""}><span className="glyphicon glyphicon-floppy-disk">&nbsp;</span>{id? "Update" : "Save"}</button>
            {/* <input type="submit" className='btn btn-primary w-10' value={id? "Update" : "Save"}/> */}
            &nbsp;&nbsp;&nbsp;&nbsp;<button className="ui button red w-20" hidden={action? "hidden" : ""} disabled={id} onClick={resetForm}><span className="glyphicon glyphicon-remove-circle"></span>&nbsp;&nbsp;Reset</button>
            <Link to ="/CitiesList">
