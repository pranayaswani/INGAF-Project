...
const CitiesList = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/get/cities");
  ...
  const deleteRecord = (id) =>{
    ...
    {
    axios.delete(`http://localhost:5000/delete/cities/${id}`)
    toast.success("Record Deleted Successfully...")    
    setTimeout(()=> getData(),500);}
  }
  const updateRecord = (id) =>{
    <Link to={`/Cities/${id}`}></Link>
    axios.get(`http://localhost:5000/get/cities/${id}`)
  }
      ...
      subHeaderComponent={
        ...
        <input
          ...
          placeholder="Type text to Search..."/>
                  <a href="/Cities" className="btn btn-success pull-right">
