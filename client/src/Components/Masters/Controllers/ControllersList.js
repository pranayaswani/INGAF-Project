...
const ControllersList = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/get/controllers");
  ...
  const deleteRecord = (id) =>{
    ...
    {
    axios.delete(`http://localhost:5000/delete/controllers/${id}`)
    toast.success("Record Deleted Successfully...")    
    setTimeout(()=> getData(),500);}
  }
  const updateRecord = (id) =>{
    <Link to={`/Controllers/${id}`}></Link>
    axios.get(`http://localhost:5000/get/controllers/${id}`)
  }
  ...
  const columns = [
    ...
    {
        name: <b>Controller Code</b>,
        selector: (row) => row.controller_code,
        sortable: true,
      },

    {
        name: <b>Controller Name</b>,
    ...
    },
    {
      ...
      cell: (row) => (
        ...
        <Link to={`/Controllers/${action}/${row.id}`}>
          <button className="btn btn-primary"  data-toggle=" tooltip" data-placement="bottom" title="View Record"><i class="fa-solid fa-eye"></i></button>                
          </Link>
        ),
    },
    {
      ...
      subHeaderComponent={
        ...
        <input
          ...
          placeholder="Type text to Search..."/>
                  <a href="/Controllers" className="btn btn-success pull-right">
