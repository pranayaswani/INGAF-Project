...
const DesignationsList = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/get/designations");
  ...
  const deleteRecord = (id) =>{
    ...
    {
    axios.delete(`http://localhost:5000/delete/designations/${id}`)
    toast.success("Record Deleted Successfully...")    
    setTimeout(()=> getData(),500);}
  }
  const updateRecord = (id) =>{
    <Link to={`/Designations/${id}`}></Link>
    axios.get(`http://localhost:5000/get/designations/${id}`)
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        ...
        <Link to={`/Designations/${action}/${row.id}`}>
      ...
      subHeaderComponent={
        ...
        <input
          ...
          placeholder="Type text to Search..."/>
                  <a href="/Designations" className="btn btn-success pull-right">
