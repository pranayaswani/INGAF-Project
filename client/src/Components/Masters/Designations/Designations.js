...
const Designations = () => {
    ...
    useEffect(()=>{
      axios.get(`http://localhost:5000/getById/designations/${id}`)
        .then((response) => {
    ...
    useEffect(()=>{
        ...
        if(Object.keys(formErrors).length===0 && isSubmit)
        {
            ...
            {
              axios.put(`http://localhost:5000/updateStatus/designations/${id}`,{
                status_id
                }).then((res)=>{
                  console.log(res);
                  if(res.status==200)
            ...
            {
              axios.get(`http://localhost:5000/get/designation_check/${descr}`)
                        .then((res1)=>{
                        ...
                        }else
                        {
                          if(id)
                          {
                            axios.put(`http://localhost:5000/update/designations/${id}`,{
                              descr,status_id
                              }).then((res)=>{
                                if(res.status==200)
                                {
                          ...
                          {
                              axios.post("http://localhost:5000/post/designations",
                              {descr,status_id})
                              .then((res)=>
                              {
                                  if(res.status==200)
