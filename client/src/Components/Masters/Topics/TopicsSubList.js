...
import TopicsSub from "./TopicsSub";
...
const TopicsSubList = () => {
  ...
  const componentRoute = "/TopicsSub";
  const componentHeader = "Topics - Sub - List"
  ...
  useEffect(() => {
    const result = tblData.filter((dt) => {
      return dt.maintype.toLowerCase().match(search.toLowerCase());
      toast.success("Record Deleted Successfully...");
    });
