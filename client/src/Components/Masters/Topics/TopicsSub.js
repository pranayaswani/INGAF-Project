...
const TopicsSub = () => {
    const apiName = "topics_sub";
    const componentRoute = "/TopicsSubList";
    const componentHeader = "Topics - Sub"
    ...
    const {descr, main_id, status_id} = state;

    const {action, id} = useParams();
     
    useEffect(()=>{
      axios.get(`http://localhost:5000/getById/${apiName}/${id}`)
        .then((response) => {
          const {id, descr, main_id, status_id} = response.data[0];
    ...
    useEffect(()=>{
        ...
        if(Object.keys(formErrors).length===0 && isSubmit)
        {
            ...
            {
              axios.put(`http://localhost:5000/updateStatus/${apiName}/${id}`,{
                status_id
                }).then((res)=>{
                  console.log(res);
                  if(res.status==200)
            ...
            {
              axios.get(`http://localhost:5000/get/${apiName}_check/${descr}`)
                        .then((res1)=>{
                        ...
                        }else
                        {
                          if(id)
                          {
                            axios.put(`http://localhost:5000/update/${apiName}/${id}`,{
                              descr,main_id,status_id
                              }).then((res)=>{
                                if(res.status==200)
                                {
                          ...
                          {
                              axios.post(`http://localhost:5000/post/${apiName}`,
                              {descr,main_id,status_id})
                              .then((res)=>
                              {
                                  if(res.status==200)
    ...
    const validate = (values) =>{
        const errors={};
        if(values.main_id==="0"){errors.main_id = "Main Topic is REQUIRED.";}        
  ...
  return (
    ...
    <form className='ui form' onSubmit={handleSubmit}> 
        <div className="ui segment">
            <Header caption = {componentHeader}/>
            <div className="field">
                <label>Main Topic</label>
                <select className="ui fluid dropdown" name="main_id" value={state.main_id || "0"} onChange={handleChange} disabled={action}>
                    <option value="0">---Select Main Topic---</option>
                    {topicData.map((tpc) => (
                    <option value={tpc.id}>{tpc.description}</option>
                    ))};
                </select>
            </div>
            <p className='error'>{formErrors.main_id}</p>                                        
