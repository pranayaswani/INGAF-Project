...
import TrainingTypesSub from "./TrainingTypesSub";
...
const TrainingTypesSubList = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/get/training_types_sub");
  ...
  const deleteRecord = (id) =>{
    ...
    {
    axios.delete(`http://localhost:5000/delete/training_types_sub/${id}`)
    toast.success("Record Deleted Successfully...")    
    setTimeout(()=> getData(),500);}
  }
  const updateRecord = (id) =>{
    <Link to={`/TrainingTypessub/${id}`}></Link>
    axios.get(`http://localhost:5000/get/training_types_sub/${id}`)
  }
  ...
  const columns = [
    ...
    {
        name: <b>Training Types - Main</b>,
        selector: (row) => row.maintype,
        sortable: true,
      },
  
    {
      name: <b>Training Types - Sub</b>,
    ...
    },
    {
      ...
      cell: (row) => (
        ...
        <Link to={`/TrainingTypesSub/${action}/${row.id}`}>
    ...
    },
    {
      ...
      width:"70px",
      cell: (row) => (
        <Link to={`/TrainingTypesSub/${row.id}`}>
      ...
      subHeaderComponent={
        ...
        <input
          ...
          placeholder="Type text to Search..."/>
                  <a href="/TrainingTypesSub" className="btn btn-success pull-right">
...
export default TrainingTypesSubList;
