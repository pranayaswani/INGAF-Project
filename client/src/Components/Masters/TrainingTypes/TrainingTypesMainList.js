...
import TrainingTypesMain from "./TrainingTypesMain";
...
const TrainingTypesMainList = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/get/training_types_main");
  ...
  const deleteRecord = (id) =>{
    ...
    {
    axios.delete(`http://localhost:5000/delete/training_types_main/${id}`)
    toast.success("Record Deleted Successfully...")    
    setTimeout(()=> getData(),500);}
  }
  const updateRecord = (id) =>{
    <Link to={`/TrainingTypesMain/${id}`}></Link>
    axios.get(`http://localhost:5000/get/training_types_main/${id}`)
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        ...
        <Link to={`/TrainingTypesMain/${action}/${row.id}`}>
    ...
    },
    {
      ...
      width:"70px",
      cell: (row) => (
        <Link to={`/TrainingTypesMain/${row.id}`}>
  ...
  return (
    ...
    <DataTable
      title="Training Types - Main - List"
      fixedHeader
      ...
      subHeaderComponent={
        ...
        <input
          ...
          placeholder="Type text to Search..."/>
                  <a href="/TrainingTypesMain" className="btn btn-success pull-right">
...
export default TrainingTypesMainList;
