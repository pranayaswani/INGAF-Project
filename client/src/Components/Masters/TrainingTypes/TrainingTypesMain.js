...
const TrainingTypesMain = () => {
    ...
    useEffect(()=>{
      axios.get(`http://localhost:5000/getById/training_types_main/${id}`)
        .then((response) => {
    ...
    useEffect(()=>{
        ...
        if(Object.keys(formErrors).length===0 && isSubmit)
        {
            ...
            {
              axios.put(`http://localhost:5000/updateStatus/training_types_main/${id}`,{
                status_id
                }).then((res)=>{
                  console.log(res);
                  if(res.status==200)
            ...
            {
              axios.get(`http://localhost:5000/get/training_types_main_check/${descr}`)
                        .then((res1)=>{
                        ...
                        }else
                        {
                          if(id)
                          {
                            axios.put(`http://localhost:5000/update/training_types_main/${id}`,{
                              descr,status_id
                              }).then((res)=>{
                                if(res.status==200)
                                {
                          ...
                          {
                              axios.post("http://localhost:5000/post/training_types_main",
                              {descr,status_id})
                              .then((res)=>
                              {
                                  if(res.status==200)
