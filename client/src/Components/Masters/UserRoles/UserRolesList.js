...
const UserRolesList = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/get/user_roles");
  ...
  const deleteRecord = (id) =>{
    ...
    {
    axios.delete(`http://localhost:5000/delete/user_roles/${id}`)
    toast.success("Record Deleted Successfully...")    
    setTimeout(()=> getData(),500);}
  }
  const updateRecord = (id) =>{
    <Link to={`/UserRoles/${id}`}></Link>
    axios.get(`http://localhost:5000/get/user_roles/${id}`)
  }
  ...
  useEffect(() => {
    const result = tblData.filter((dt) => {
      return dt.role.toLowerCase().match(search.toLowerCase());
      toast.success("Record Deleted Successfully...");
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        ...
        <Link to={`/UserRoles/${action}/${row.id}`}>
