...
const ExpenditureHeads = () => {
    ...
    useEffect(()=>{
      axios.get(`http://localhost:5000/getById/expenditure_heads/${id}`)
        .then((response) => {
    ...
    useEffect(()=>{
        ...
        if(Object.keys(formErrors).length===0 && isSubmit)
        {
            ...
            {
              axios.put(`http://localhost:5000/updateStatus/expenditure_heads/${id}`,{
                status_id
                }).then((res)=>{
                  console.log(res);
                  if(res.status==200)
            ...
            {
              axios.get(`http://localhost:5000/get/expenditure_head_check/${descr}`)
                        .then((res1)=>{
                        ...
                        }else
                        {
                          if(id)
                          {
                            axios.put(`http://localhost:5000/update/expenditure_heads/${id}`,{
                              descr,status_id
                              }).then((res)=>{
                                if(res.status==200)
                                {
                          ...
                          {
                              axios.post("http://localhost:5000/post/expenditure_heads",
                              {descr,status_id})
                              .then((res)=>
                              {
                                  if(res.status==200)
  ...
  return (
    ...
    <form className='ui form' onSubmit={handleSubmit}> 
        <div className="ui segment">
            <Header caption = "Expenditure Heads"/>
            <div className="field">
                <label>Expenditure Heads</label>
...
export default ExpenditureHeads;